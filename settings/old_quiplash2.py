# game
PATH = r'C:\Program Files (x86)\Steam\steamapps\common\The Jackbox Party Pack 3\games\Quiplash2'

# in-game
# PATH_SUGGESTIONS = PATH + r'\content\AwShirtDrawingSuggestions.jet'
# PATH_SLOGAN_SUGGESTIONS = PATH + r'\content\AwShirtSloganSuggestions.jet'
# PATH_SLOGANS = PATH + r'\content\AwShirtSlogans.jet'

PATH_QUESTIONS_DIR = PATH + r'\content\QuiplashQuestion'
PATH_QUESTIONS_JSON = PATH + r'\content\QuiplashQuestion.jet'
PATH_AUDIENCE_JSON = PATH + r'\content\AudienceQuestion.jet'
PATH_LEADERBOARDS = PATH + r'\leaderboards.jet'
PATH_MENU = PATH + r'\menu.jet'
PATH_SETTINGS = PATH + r'\settings.jet'
PATH_SAFETY_QUIPS = PATH + r'\content\SafetyQuips.jet'
PATH_WORD_LASH = PATH + r'\content\WordLash.jet'
PATH_WORD_LASH_WORD = PATH + r'\content\WordLashWord.jet'
PATH_ACRO_LASH = PATH + r'\content\AcroLash.jet'
PATH_COMIC_LASH = PATH + r'\content\ComicLash.jet'
PATH_AUDIO = PATH + r'\TalkshowExport\project\media'

# encoded
PATH_DATA = '../data/pp3/quiplash2/'
PATH_ENCODED = PATH_DATA + 'encoded/'
# PATH_EXPANDED = PATH_DATA + 'swf/expanded.json'
# PATH_SOURCE_DICT = PATH_DATA + 'swf/dict.txt'
# PATH_EDITABLE_DICT = PATH_DATA + 'swf/editable.txt'
# PATH_TRANSLATED_DICT = PATH_DATA + r'swf/translated_dict.txt'

# build
PATH_BUILD = '../build/uk/JPP3/Quiplash2/'
PATH_BUILD_EXTRA_QUESTIONS = PATH_BUILD + 'extra_questions.json'
# PATH_BUILD_GAME = PATH_BUILD + 'in-game/'
# PATH_BUILD_SLOGANS = PATH_BUILD_GAME + 'slogans.json'
# PATH_BUILD_SUGGESTIONS = PATH_BUILD_GAME + 'suggestions.json'
# PATH_BUILD_MODERATED_SLOGANS = PATH_BUILD_GAME + 'moderated_slogans.json'
# PATH_BUILD_SLOGAN_SUGGESTIONS = PATH_BUILD_GAME + 'slogan_suggestions.json'
# PATH_BUILD_AUDIO = PATH_BUILD + r'audio_subtitles.json'
# PATH_BUILD_SUBTITLES = PATH_BUILD + r'text_subtitles.json'
